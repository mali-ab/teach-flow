      ...
      >
        {participants.map((participant) => {
          ...
          return (
            <div
              key={participant.id}
              className={`relative rounded-2xl overflow-hidden bg-slate-900/90 border transition-all duration-200 flex items-center justify-center min-h-[140px] shadow-lg ${
                participant.isSpeaking
                  ? "border-emerald-500 ring-2 ring-emerald-500/30"
                  : "border-slate-800/80 hover:border-slate-700"
              }`}
            >
              {/* Embedded Video Feed or Avatar Placeholder */}
              {hideVideo ? (
                ...
                <div className="w-full h-full bg-slate-800/30 flex items-center justify-center relative">
                  {/* Jitsi / WebRTC Stream mounts inside this container */}
                  <div className="text-xs text-slate-500 font-mono select-none">
                    [Video Feed: {participant.name}]
                  </div>
                </div>
              )}

              {/* Active Speaker / Participant Label Overlay */}
